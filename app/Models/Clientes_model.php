...
class Clientes_model extends Model
{
	...
	public function getClientesDeuda($where = array(), $select = '')
	{
		...
		if (!empty($select)) {
			...
			$clientes->select('v.cliente_id as id, c.nombre as nombre, count(*) as total_ventas, sum(v.total_venta) as total_venta, sum(v.total_pagado) as total_pagado, (sum(v.total_venta) - sum(v.total_pagado)) as total_deuda');
		}
